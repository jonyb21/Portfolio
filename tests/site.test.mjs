...
try {
  ...
  assert.equal(arcLock.title, "Arc Hinged D-Lock");
  assert.match(arcLock.type, /hinged D-lock/i, "Arc lock names its actual product architecture");
  assert.match(arcLock.notes.join(" "), /concealed permanent pivot/i, "Arc lock documents its captive pivot");
  assert.match(arcLock.notes.join(" "), /recessed receiver.*internal deadbolt/i, "Arc lock documents its working receiver");
  assert.match(arcLock.notes.join(" "), /frame mount.*transport/i, "Arc lock separates transport from security");
  assert.doesNotMatch(arcLockCopy, /folding link|steel chain|removable shackle|detached shackle/i, "Arc lock never describes the rejected folding or loose-part concepts");
  assert.equal(new Set(arcLockImages).size, 9, "Arc lock uses nine distinct portfolio images");
  assert(arcLockImages.every(image => /^\/assets\/mobility\/arc-hinged-d-lock-[a-z-]+-photo-v[67]\.webp$/.test(image)), "Arc lock uses the coherent versioned media set");
  const arcMountView = arcLock.views.find(view => view.image.includes("context-wide"));
  const arcClosingView = arcLock.views.find(view => view.image.includes("context-active"));
  assert.equal(arcMountView.label, "Closed lock releasing from frame mount", "Transport view describes the lock and mount as separate parts");
  assert(arcMountView.image.endsWith("context-wide-photo-v7.webp"), "Transport view uses the corrected frame-mount render");
  assert.equal(arcClosingView.label, "Shackle closing around frame and fixed rack", "Locking view identifies both secured structures");
  assert(arcClosingView.image.endsWith("context-active-photo-v7.webp"), "Locking view uses the corrected secure-locking render");
  assert.equal(crypto.createHash("sha256").update(fs.readFileSync(`public${arcMountView.image}`)).digest("hex"), "65f227cb59c7bdca7af73ad66de79efa5892b0a33fa239276c4720a1194d2b26", "Transport view preserves the approved separate-mount action");
  assert.equal(crypto.createHash("sha256").update(fs.readFileSync(`public${arcClosingView.image}`)).digest("hex"), "c90935b76fc397c6b141f1511d4d482c9887d93edc361496f612d8599589087e", "Locking view preserves the approved frame-and-rack action");
  ...
  const electricPumpCopy = `${electricPump.summary} ${electricPump.notes.join(" ")} ${electricPump.views.map(view => view.label).join(" ")}`;
  const electricPumpImages = [electricPump.image, ...electricPump.views.map(view => view.image)];
  assert.equal(electricPump.title, "Gauge Electric Pump");
  assert.match(electricPump.summary, /LED work light.*air-intake panel/i, "Pump locates its work light in the side air-intake panel");
  assert.match(electricPump.notes.join(" "), /sealed LED module.*perforated side air-intake/i, "Pump documents the intake-mounted light as a serviceable component");
  assert.doesNotMatch(electricPumpCopy, /nozzle-tip LED|LED.*nozzle|nozzle.*LED|LED ring/i, "Pump never describes the rejected nozzle-mounted light");
  assert(electricPumpImages.every(image => /^\/assets\/mobility\/gauge-electric-pump-[a-z-]+-photo-v[456]\.webp$/.test(image)), "Pump uses the corrected versioned product gallery");
  assert.equal(electricPump.views.find(view => view.image.includes("detail-primary")).label, "Air-intake work light");
  assert.equal(electricPump.views.find(view => view.image.includes("context-alt")).label, "Kerbside inflation with one connected valve");
  const electricPumpHumanView = electricPump.views.find(view => view.image.includes("context-active"));
  assert.equal(electricPumpHumanView.label, "Human-operated inflation with intake facing valve");
  assert(electricPumpHumanView.image.endsWith("context-active-photo-v6.webp"), "Pump includes the physically aligned human-operated light-use render");
  assert.match(electricPump.notes.join(" "), /intake panel faces the valve during use/i, "Pump documents the required line of sight from its fixed light");
  assert.equal(crypto.createHash("sha256").update(fs.readFileSync("public/assets/mobility/gauge-electric-pump-detail-primary-photo-v4.webp")).digest("hex"), "91fd63d10285227b96ebe7c771a5dfcb3fa02be47cb90a135d60d851920206ed", "Pump detail preserves the approved intake-mounted light");
  assert.equal(crypto.createHash("sha256").update(fs.readFileSync("public/assets/mobility/gauge-electric-pump-context-alt-photo-v4.webp")).digest("hex"), "274216fab4ecbc83781daae4cd0bc03f67dc4fb3b5652d8fdb1603403e821566", "Pump kerbside view preserves the approved single-valve connection");
  assert.equal(crypto.createHash("sha256").update(fs.readFileSync("public/assets/mobility/gauge-electric-pump-context-active-photo-v6.webp")).digest("hex"), "ec897921cd50a5e4cfe14898ec928c7cdbc523f67efc5e352043da6b0c31925d", "Pump human-use view preserves the approved intake-to-valve line of sight");
