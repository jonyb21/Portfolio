...
try {
  ...
  assert(site.projects.every(project => project.views.filter(view => view.type === "insitu").every(view => /-(?:insitu-v(?:[1-5]|4-fixed)|in-use-v1|context-(?:wide|alt|active|use)-(?:vibrant-v1|photo-v[23456]))\.webp$/.test(view.image))));
  assert(site.projects.filter(project => project.category !== "furniture").every(project => project.views.some(view => view.type === "insitu" && /-context-use-(?:vibrant-v1|photo-v[23456])\.webp$/.test(view.image))), "Every Homewares, Lighting, and Mobility project includes a use scene");
  ...
  assert(photoProjects.every(project => [project.image, project.cardImage, project.detailImage, ...project.views.map(view => view.image)].every(image => /-photo-v[23456]\.webp$/.test(image))), "New and corrected projects use the realistic versioned photo media system");
  ...
  assert(arcLockImages.every(image => /^\/assets\/mobility\/arc-hinged-d-lock-[a-z-]+-photo-v6\.webp$/.test(image)), "Arc lock uses the coherent photo-v6 media set");
