...
const MEDIA_REVISION = "20260718-6";
...
try {
  ...
  assert(site.projects.every(project => project.views.filter(view => view.type === "insitu").every(view => /-(?:insitu-v(?:[1-5]|4-fixed)|in-use-v1|context-(?:wide|alt|active|use)-(?:vibrant-v1|photo-v[2345]))\.webp$/.test(view.image))));
  assert(site.projects.filter(project => project.category !== "furniture").every(project => project.views.some(view => view.type === "insitu" && /-context-use-(?:vibrant-v1|photo-v[2345])\.webp$/.test(view.image))), "Every Homewares, Lighting, and Mobility project includes a use scene");
  ...
  assert(photoProjects.every(project => [project.image, project.cardImage, project.detailImage, ...project.views.map(view => view.image)].every(image => /-photo-v[2345]\.webp$/.test(image))), "New and corrected projects use the realistic versioned photo media system");
  const aeroHelmet = site.projects.find(project => project.slug === "aero-commuter-helmet");
  assert([aeroHelmet.image, aeroHelmet.cardImage, aeroHelmet.views[1].image, aeroHelmet.views[3].image, aeroHelmet.views[7].image].every(image => image.endsWith("-photo-v5.webp")), "Aero helmet uses the seated, indicator, and clasp photo-v5 assets");
