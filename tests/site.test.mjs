...
try {
  ...
  assert(electricPumpImages.every(image => /^\/assets\/mobility\/gauge-electric-pump-[a-z-]+-photo-v[45]\.webp$/.test(image)), "Pump uses the corrected versioned product gallery");
  ...
  assert.equal(electricPumpHumanView.label, "Human-operated inflation at dusk");
  assert(electricPumpHumanView.image.endsWith("context-active-photo-v5.webp"), "Pump includes the approved human-operated light-use render");
  ...
  assert.equal(crypto.createHash("sha256").update(fs.readFileSync("public/assets/mobility/gauge-electric-pump-context-active-photo-v5.webp")).digest("hex"), "a717fec29e0f31bcc6d1d2b8e686e942a556862c236c834ac33e1eca04ac199d", "Pump dusk view preserves the approved human-operated intake light");
