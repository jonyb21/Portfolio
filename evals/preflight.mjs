...
assert(site.projects.every(project => project.views.filter(view => view.type === "insitu").every(view => /-(?:insitu-v(?:[1-5]|4-fixed)|in-use-v1|context-(?:wide|alt|active|use)-(?:vibrant-v1|photo-v[2345678]))\.webp$/.test(view.image))), "Each project uses four optimized generated in-situ assets");
...
assert(electricPumpKerbsideView.label === "Human-operated kerbside inflation" && electricPumpKerbsideView.image.endsWith("context-alt-photo-v8.webp"), "The pump kerbside card includes a person using the nozzle-lit valve connection");
assert(localAssetHash("/assets/mobility/gauge-electric-pump-context-alt-photo-v8.webp") === "770674ee60f298a6b7ca7bdaf38de58af2ea495c572f2961e4307bcb6871ce8c", "The pump kerbside card preserves the approved human-operated nozzle light");
