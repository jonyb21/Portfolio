...
assert(site.projects.every(project => project.views.filter(view => view.type === "insitu").every(view => /-(?:insitu-v(?:[1-5]|4-fixed)|in-use-v1|context-(?:wide|alt|active|use)-(?:vibrant-v1|photo-v[234567]))\.webp$/.test(view.image))), "Each project uses four optimized generated in-situ assets");
const generatedProjects = site.projects.filter(project => project.category !== "furniture" || project.slug === "pivot-writing-desk");
assert(generatedProjects.every(project => project.views.some(view => view.type === "insitu" && /-context-use-(?:vibrant-v1|photo-v[234567])\.webp$/.test(view.image))), "Every generated product story includes a use scene");
...
assert(arcLock.title === "Arc Hinged D-Lock", "Arc lock uses the approved product name");
assert(/hinged D-lock/i.test(arcLock.type), "Arc lock names its actual product architecture");
assert(/concealed permanent pivot/i.test(arcLock.notes.join(" ")), "Arc lock documents its captive pivot");
assert(/recessed receiver.*internal deadbolt/i.test(arcLock.notes.join(" ")), "Arc lock documents its working receiver");
assert(/frame mount.*transport/i.test(arcLock.notes.join(" ")), "Arc lock separates transport from security");
assert(!/folding link|steel chain|removable shackle|detached shackle/i.test(arcLockCopy), "Arc lock rejects folding and loose-part terminology");
...
assert(arcMountView.label === "Closed lock releasing from frame mount", "Transport view describes the lock and mount as separate parts");
assert(arcMountView.image.endsWith("context-wide-photo-v7.webp"), "Transport view uses the corrected frame-mount render");
assert(arcClosingView.label === "Shackle closing around frame and fixed rack", "Locking view identifies both secured structures");
assert(arcClosingView.image.endsWith("context-active-photo-v7.webp"), "Locking view uses the corrected secure-locking render");
assert(localAssetHash(arcMountView.image) === "65f227cb59c7bdca7af73ad66de79efa5892b0a33fa239276c4720a1194d2b26", "Transport view preserves the approved separate-mount action");
assert(localAssetHash(arcClosingView.image) === "c90935b76fc397c6b141f1511d4d482c9887d93edc361496f612d8599589087e", "Locking view preserves the approved frame-and-rack action");
...
assert(electricPump.title === "Gauge Electric Pump" && /LED work light.*air-intake panel/i.test(electricPump.summary), "The electric pump locates its work light in the side air-intake panel");
assert(/sealed LED module.*perforated side air-intake/i.test(electricPump.notes.join(" ")), "The electric pump documents the intake-mounted light as a serviceable component");
assert(!/nozzle-tip LED|LED.*nozzle|nozzle.*LED|LED ring/i.test(electricPumpCopy), "The electric pump rejects the old nozzle-mounted light throughout its product story");
assert(electricPumpImages.every(image => /^\/assets\/mobility\/gauge-electric-pump-[a-z-]+-photo-v[456]\.webp$/.test(image)), "The electric pump uses one corrected versioned gallery");
assert(electricPump.views.find(view => view.image.includes("detail-primary")).label === "Air-intake work light", "The pump detail view identifies the intake-mounted light");
assert(electricPump.views.find(view => view.image.includes("context-alt")).label === "Kerbside inflation with one connected valve", "The kerbside view documents one physical valve connection");
const electricPumpHumanView = electricPump.views.find(view => view.image.includes("context-active"));
assert(electricPumpHumanView.label === "Human-operated inflation with intake facing valve" && electricPumpHumanView.image.endsWith("context-active-photo-v6.webp"), "The pump includes a human operating its physically aligned intake-lit valve connection");
assert(/intake panel faces the valve during use/i.test(electricPump.notes.join(" ")), "The pump documents the required line of sight from its fixed light");
assert(localAssetHash("/assets/mobility/gauge-electric-pump-detail-primary-photo-v4.webp") === "91fd63d10285227b96ebe7c771a5dfcb3fa02be47cb90a135d60d851920206ed", "The pump detail preserves the approved intake-mounted light");
assert(localAssetHash("/assets/mobility/gauge-electric-pump-context-alt-photo-v4.webp") === "274216fab4ecbc83781daae4cd0bc03f67dc4fb3b5652d8fdb1603403e821566", "The pump kerbside view preserves the approved single-valve connection");
assert(localAssetHash("/assets/mobility/gauge-electric-pump-context-active-photo-v6.webp") === "ec897921cd50a5e4cfe14898ec928c7cdbc523f67efc5e352043da6b0c31925d", "The pump human-use view preserves the approved intake-to-valve line of sight");
