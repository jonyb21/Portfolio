...
assert(site.projects.every(project => project.views.filter(view => view.type === "insitu").every(view => /-(?:insitu-v(?:[1-5]|4-fixed)|in-use-v1|context-(?:wide|alt|active|use)-(?:vibrant-v1|photo-v[234]))\.webp$/.test(view.image))), "Each project uses four optimized generated in-situ assets");
const generatedProjects = site.projects.filter(project => project.category !== "furniture" || project.slug === "pivot-writing-desk");
assert(generatedProjects.every(project => project.views.some(view => view.type === "insitu" && /-context-use-(?:vibrant-v1|photo-v[234])\.webp$/.test(view.image))), "Every generated product story includes a use scene");
